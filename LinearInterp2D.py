...
# ================================================================ #
def LinearInterp2D( gridData    =None , pointData=None, size=None, gridDataType="structured", \
                    gridDataOnly=False, unstructuredOrder="ijk" ):
    # ---------------------------------------- #
    # --- [1]   引数チェック               --- #
    # ---------------------------------------- #
    if ( gridData  is None ): sys.exit( "[LinearInterp2D] gridData  ???" )
    if ( pointData is None ): sys.exit( "[LinearInterp2D] pointData ???" )
    ...
    if   ( gridDataType == "structured"   ):
        if ( gridDataOnly ):
            ...
            if ( ( dx is None ) or ( dy is None ) or ( size is None ) ):
                ...
                print( "[LinearInterp2D] gridDataType == unstructured, with gridDataOnly == True" )
                print( "[LinearInterp2D]           dx == {0}".format(   dx ) )
                print( "[LinearInterp2D]           dy == {0}".format(   dy ) )
                print( "[LinearInterp2D]         size == {0}".format( size ) )
                sys.exit()
            else:
        ...
        if ( size is None ):
            print( "[LinearInterp2D] gridDataType == unstructured, but no size !!" )
            print( "[LinearInterp2D]         size == ???" )
            sys.exit()
        else:
            LI, LJ   = size[0], size[1]
...
if ( __name__=='__main__' ):
    ...
    ret = LinearInterp2D( gridData=gridData, pointData=pointData, \
