...
def interpolate__triElement( nodes=None, points=None ):
    ...
    # --- [1]   引数チェック               --- #
    # ---------------------------------------- #
    if ( nodes  is None ): sys.exit( "[interpolate__triElement] nodes  ???" )
    if ( points is None ): sys.exit( "[interpolate__triElement] points ???" )
...
if ( __name__=="__main__" ):
    ...
    ret = interpolate__triElement( nodes=gData, points=points )
