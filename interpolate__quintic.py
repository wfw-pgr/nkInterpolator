...
# ========================================================= #
# ===  interpolate__quintic                             === #
# ========================================================= #

def interpolate__quintic( xp=None, xd=None, fx=None, fpx=None, fppx=None, \
    ...
    if ( nPoint * nEqType != nCoef ):
        print( "[interpolate__quintic.py] information is not sufficient.... [ERROR]" )
        print( "[interpolate__quintic.py] order       ::  {0}".format( order       ) )
        print( "[interpolate__quintic.py]  => nCoef   ::  {0}".format( nCoef       ) )
        print()
        print( "[interpolate__quintic.py] nPoint      ::  {0}".format( nPoint      ) )
        print()
        print( "[interpolate__quintic.py] nDerivative ::  {0}".format( nDerivative ) )
        print( "[interpolate__quintic.py]  => nEqType ::  {0}".format( nEqType     ) )
        print( "[interpolate__quintic.py] nCoef != nEqType * nPoint !!! [ERROR]" )
        sys.exit()
    ...
    if ( lhs.shape[0] != nCoef ):
        print( "[interpolate__quintic.py] data        is not sufficient.... [ERROR]" )
        if ( nDerivative >= 0 ):
            print( "[interpolate__quintic.py] xd.shape   ::  {0}".format( xd.shape   ) )
            print( "[interpolate__quintic.py] fx.shape   ::  {0}".format( fx.shape   ) )
        if ( nDerivative >= 1 ):
            print( "[interpolate__quintic.py] fpx.shape  ::  {0}".format( fpx.shape  ) )
        if ( nDerivative >= 2 ):
            print( "[interpolate__quintic.py] fppx.shape ::  {0}".format( fppx.shape ) )
...
if ( __name__=="__main__" ):
    ...
    # ret = interpolate__quintic( xp=xp, xd=xd, fx=fx, fpx=fpx, nDerivative=1, order=5 )
    ...
    ret = interpolate__quintic( xp=xp, xd=xd, fx=fx, fpx=fpx, fppx=fppx, nDerivative=2, order=5 )
    ...
    # ret = interpolate__quintic( xp=xp, xd=xd, fx=fx, fpx=fpx, nDerivative=1, order=3 )
