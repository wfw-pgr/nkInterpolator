...
# ================================================================ #
def interpolate__bilinear( gridData    =None , pointData=None, size=None, gridDataType="structured", \
                           gridDataOnly=False, unstructuredOrder="ijk" ):
    # ---------------------------------------- #
    # --- [1]   引数チェック               --- #
    # ---------------------------------------- #
    if ( gridData  is None ): sys.exit( "[interpolate__bilinear] gridData  ???" )
    if ( pointData is None ): sys.exit( "[interpolate__bilinear] pointData ???" )
    ...
    if   ( gridDataType == "structured"   ):
        if ( gridDataOnly ):
            ...
            if ( ( dx is None ) or ( dy is None ) or ( size is None ) ):
                ...
                print( "[interpolate__bilinear] gridDataType == unstructured, with gridDataOnly == True" )
                print( "[interpolate__bilinear]           dx == {0}".format(   dx ) )
                print( "[interpolate__bilinear]           dy == {0}".format(   dy ) )
                print( "[interpolate__bilinear]         size == {0}".format( size ) )
                sys.exit()
            else:
        ...
        if ( size is None ):
            print( "[interpolate__bilinear] gridDataType == unstructured, but no size !!" )
            print( "[interpolate__bilinear]         size == ???" )
            sys.exit()
        else:
            LI, LJ   = size[0], size[1]
...
if ( __name__=='__main__' ):
    ...
    ret = interpolate__bilinear( gridData=gridData, pointData=pointData, \
